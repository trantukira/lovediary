  ...
  for (let i = 0; i <= backoff.length; i++) {
    try {
      ...
      if (i === backoff.length) {
        ...
        return "Đã có lỗi xảy ra. Thử lại sau nhé!";
      }
...
export default function App() {
    ...
    const initAuth = async () => {
      try {
        ...
        if (
          ...
          setAuthError(
            "Bạn chưa bật quyền Đăng nhập Ẩn danh (Anonymous) trên Firebase!"
          );
  ...
  const updateSettings = async (newProfiles, newStartDate) => {
    if (!user) return;
    await setDoc(
      doc(db, "love_settings", "main"),
      {
  ...
  const handleProfileChange = (person, field, value) => {
  ...
  const handleProfileImageUpload = async (person, e) => {
  ...
  const handleToggleBucket = async (id, currentStatus) => {
    if (user) {
      await setDoc(
        doc(db, "love_bucketList", id),
        {
  ...
  const handleDownloadImage = (e, memory) => {
    e.stopPropagation();
  ...
  const handleGenerateMessage = async () => {
    if (!newWish.author) {
      setNewWish((prev) => ({
        ...prev,
        text: "Hãy chọn tên bạn trước khi nhờ AI viết nhé! 💕",
      }));
      return;
    }
        ...
        <div className="bg-white/90 backdrop-blur-md p-8 md:p-10 rounded-[2.5rem] shadow-2xl max-w-lg w-full border border-red-100 text-center animate-in zoom-in-95">
          ...
          <div className="text-left bg-red-50/80 p-6 rounded-3xl text-sm text-red-900 space-y-3 border border-red-200">
            <p className="font-bold text-base flex items-center">
              <CheckCircle className="w-5 h-5 mr-2" /> Cách khắc phục nhanh:
            </p>
            <ol className="list-decimal list-inside space-y-2 ml-1">
              <li>
                Mở tab mới, vào <b>Firebase Console</b>.
              </li>
              <li>
                Truy cập dự án <b>love-diary-3c82d</b> của bạn.
              </li>
              <li>
                Chọn menu <b>Authentication</b> bên trái.
              </li>
              <li>
                Chuyển sang tab <b>Sign-in method</b>.
              </li>
              <li>
                Tìm mục <b>Anonymous</b> và chuyển sang{" "}
  ...
  if (!isDbReady) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-pink-50">
        ...
        <p className="text-pink-500 font-bold text-lg animate-pulse tracking-wide">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-blue-50 to-purple-100 font-sans text-gray-800 pb-20">
      ...
      {/* NAVBAR */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-md border-b border-white/40 shadow-sm px-4 md:px-8 py-3 md:py-4 flex justify-between items-center transition-all h-16 md:h-20">
        <div
          className="flex items-center cursor-pointer h-full"
          onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
        >
          {/* LOGO GỐC ĐÃ ĐƯỢC CẮT CỦA BẠN */}
          <img
            src="/logo.png"
            alt="Logo"
            className="w-12 h-12 md:w-14 md:h-14 object-contain mr-3"
            onError={(e) => {
              e.target.onerror = null;
              e.target.src =
                "https://placehold.co/100x100/ff6b6b/white?text=Logo";
            }}
          />
        ...
        <div className="hidden md:flex space-x-8 text-sm md:text-base font-bold text-gray-600">
          <a href="#profiles" className="hover:text-pink-500 transition-colors">
            Profile
          </a>
          <a href="#wishes" className="hover:text-pink-500 transition-colors">
            Messages
          </a>
          <a href="#memories" className="hover:text-pink-500 transition-colors">
            Memories
          </a>
          <a
            href="#bucketlist"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 pt-28 md:pt-36">
        ...
        <div className="text-center mb-12">
          <h1
            className="text-5xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500 mb-4 drop-shadow-sm pb-2"
            style={{ fontFamily: "cursive, sans-serif" }}
          >
            Chuyện đôi mình 💕
          </h1>
          <p className="text-gray-500 font-medium tracking-wide">
            Khắc ghi từng khoảnh khắc, gói trọn cả yêu thương ✨
          </p>
        </div>
        ...
        <div
          id="profiles"
          className="relative grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-12 mb-16"
        >
          {["person1", "person2"].map((personKey) => (
            <div
              key={personKey}
              className="bg-white/60 hover:bg-white/80 transition-all duration-300 backdrop-blur-md rounded-[2.5rem] p-8 shadow-xl border border-white/60 relative overflow-hidden group hover:-translate-y-1"
            >
              <div className="absolute top-0 right-0 p-5 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={() =>
                    setEditingProfile(
                      editingProfile === personKey ? null : personKey
                    )
                  }
                  ...
                  {editingProfile === personKey && (
                    <label className="absolute bottom-2 right-2 bg-pink-500 text-white p-2.5 rounded-full cursor-pointer hover:bg-pink-600 shadow-lg transition-transform hover:scale-110">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleProfileImageUpload(personKey, e)}
                      />
                    </label>
                  )}
                </div>
                ...
                {editingProfile === personKey ? (
                  <div className="w-full space-y-3 mt-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleProfileChange(personKey, "name", e.target.value)
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleProfileChange(personKey, "dob", e.target.value)
                      }
                  ...
                  <>
                    ...
                    <div className="flex flex-col items-center text-gray-600 space-y-1.5 mt-2 bg-white/50 px-6 py-3.5 rounded-3xl w-full border border-white/40">
                      <div className="flex items-center font-medium">
                        <Calendar className="w-4 h-4 mr-2 text-blue-500" /> Sinh
                        nhật:{" "}
                        {new Date(profiles[personKey].dob).toLocaleDateString(
                          "vi-VN"
                        )}
                      </div>
                      <div className="flex items-center font-medium">
                        <Sparkles className="w-4 h-4 mr-2 text-yellow-500" />{" "}
                        {getZodiacSign(profiles[personKey].dob)}
                      </div>
                    </div>
                  </>
                )}
        ...
        <div className="text-center mb-16 relative">
          <div className="inline-block bg-white/70 backdrop-blur-md rounded-[3rem] px-10 py-8 shadow-2xl border border-white/60 transform hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-center space-x-3 mb-2">
              ...
              <span className="text-xl font-semibold text-gray-600 uppercase tracking-wider">
                Đã bên nhau
              </span>
              <Heart className="text-red-400 animate-pulse w-6 h-6 fill-current opacity-80" />
            </div>
            ...
            <div className="text-xl text-gray-500 font-bold tracking-widest uppercase">
              Ngày
            </div>
        ...
        <div
          id="wishes"
          className="bg-white/70 backdrop-blur-md rounded-[2.5rem] p-8 shadow-xl border border-white/60 mb-16 hover:shadow-2xl transition-shadow duration-300"
        >
          <h2 className="text-3xl font-bold text-gray-800 mb-8 flex items-center">
            <Heart className="w-7 h-7 mr-3 text-pink-500 fill-current" /> Lời
            nhắn gửi cho nhau
          </h2>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8 bg-pink-50/70 p-5 rounded-[2rem] border border-pink-100 shadow-inner">
            ...
            >
              ...
              <option value={profiles.person1.name}>
                {profiles.person1.name}
              </option>
              <option value={profiles.person2.name}>
                {profiles.person2.name}
              </option>
            </select>
            ...
            <div className="flex-1 relative flex items-center">
              <input
                ...
                value={newWish.text}
                onChange={(e) =>
                  setNewWish({ ...newWish, text: e.target.value })
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {wishes.map((wish) => (
              <div
                key={wish.id}
                className="bg-white/90 p-5 rounded-[2rem] shadow-sm border border-gray-100 relative group hover:shadow-md transition-all hover:-translate-y-1"
              >
                <button
                ...
                <div className="font-bold text-pink-500 mb-2 text-lg">
                  {wish.author}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  {newMemory.imageUrl ? (
                    ...
                    <>
                      ...
                      <p className="text-sm font-medium text-gray-500 mb-4">
                        Tải ảnh kỉ niệm từ thiết bị
                      </p>
                    </>
                  )}
                  <label className="bg-gradient-to-r from-pink-400 to-pink-500 text-white px-6 py-2.5 rounded-full cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all relative z-10 font-medium">
                    Chọn ảnh
                    <input
                      type="file"
                      accept="image/*"
                ...
                <div className="space-y-5">
                  ...
                  <button
                    ...
                    disabled={
                      !newMemory.imageUrl || !newMemory.date || !newMemory.text
                    }
            ...
            {memories.map((memory) => (
              ...
              >
                ...
                <div className="p-6 flex-1 flex flex-col bg-gradient-to-b from-white/50 to-white/10">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm font-medium line-clamp-3">
                    {memory.text}
                  </p>
                </div>
              </div>
        ...
        <div
          id="bucketlist"
          className="bg-white/70 backdrop-blur-md rounded-[2.5rem] p-8 shadow-xl border border-white/60 hover:shadow-2xl transition-shadow duration-300"
        >
          ...
          <p className="text-gray-500 mb-6 text-base font-medium ml-1">
            Những điều chúng mình sẽ cùng nhau thực hiện ✈️
          </p>
          ...
          <div className="space-y-4 mb-8">
            {bucketList.map((item) => (
              <div
                key={item.id}
                className={`flex items-center justify-between p-4 rounded-2xl transition-all duration-300 border shadow-sm group hover:-translate-y-0.5 ${
                  item.completed
                    ? "bg-green-50/70 border-green-200"
                    : "bg-white/80 border-white hover:border-pink-200 hover:shadow-md"
                }`}
              >
                <div
                  className="flex items-center cursor-pointer flex-1"
                  onClick={() => handleToggleBucket(item.id, item.completed)}
                >
                  <div
                    className={`w-6 h-6 rounded-full border-[2.5px] flex items-center justify-center mr-4 transition-colors duration-300 ${
                      item.completed
                        ? "bg-green-400 border-green-400 shadow-sm"
                ...
                <button
                  ...
                  className="text-gray-300 hover:text-red-400 p-2 rounded-full hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                >
            ...
            >
              ...
              <div className="w-full md:w-3/5 bg-black/5 flex items-center justify-center min-h-[40vh] md:min-h-[60vh] relative group overflow-hidden">
                ...
                <img
                  src={viewingMemory.imageUrl}
                  alt="Memory Zoom"
                  className="max-w-full max-h-[50vh] md:max-h-[85vh] object-contain relative z-10 shadow-2xl"
                />
              </div>
              ...
              <div className="w-full md:w-2/5 p-8 md:p-10 flex flex-col bg-white overflow-y-auto">
                ...
                <div className="flex items-center text-sm font-bold text-pink-500 mb-6 bg-pink-50 inline-flex px-4 py-2 rounded-2xl self-start shadow-sm border border-pink-100">
                  ...
                  {new Date(viewingMemory.date).toLocaleDateString("vi-VN")}
                </div>
                <p className="text-gray-700 text-lg leading-relaxed flex-1 mb-8 whitespace-pre-line">
                  {viewingMemory.text}
                </p>
        ...
        <div className="mt-16 text-center text-sm font-medium text-gray-500 pb-8 flex flex-col items-center justify-center space-y-3 relative z-10">
          ...
          <span className="bg-white/60 backdrop-blur-sm px-5 py-2 rounded-full shadow-sm border border-white/60 transition-transform hover:-translate-y-1">
            dev by{" "}
            <strong className="text-pink-500 font-extrabold text-base">
              Trantus🦖
            </strong>
          </span>
        </div>
